  ...
  return (
    <div className="flex flex-row sp:flex-col gap-10 items-start justify-start max-w-[920px]">
      <div className="flex flex-col gap-4 py-2 flex-1 sp:order-2">
        <span className=" font-light text-lg leading-6">{number}</span>
        <h3 className="text-xl leading-8 whitespace-pre-line">{title}</h3>
        <p className="body4 whitespace-pre-line">{description}</p>
        {note && (
          <p className="text-[10px] leading-[18px] opacity-50">{note}</p>
        )}
      </div>
