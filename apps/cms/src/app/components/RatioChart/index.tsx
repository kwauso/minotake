...
export const RatioChart = () => {
  ...
  const getLabelPosition = (
    ...
    const distance =
      radius +
      currentStrokeWidth +
      (Math.abs(Math.sin(angle - Math.PI / 2)) > 0.5
        ? labelDistance.y
        : labelDistance.x);

    return {
      x:
        Math.round((center + distance * Math.cos(angle - Math.PI / 2)) * 100) /
        100,
      y:
        Math.round((center + distance * Math.sin(angle - Math.PI / 2)) * 100) /
        100,
    };
  };
              ...
              {participants.map((participant, index) => (
                <motion.path
                  ...
                  d={describeArc(
                    ...
                    center,
                    radius,
                    participant.startAngle,
                    participant.endAngle
                  )}
                  fill="none"
